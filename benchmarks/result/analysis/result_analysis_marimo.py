...
def __(add_num_annotation, plt, sns):
    ...
    plt.show()
    return bar_plot_1, required_metric_name
    ...
    for _dir_name in execute_result_dir_names:
        for _file in os.listdir(_dir_name):
            if (
                ...
                and "10.0qps" in _file      # Need to change
            ):
...
@app.cell
def __(execute_result_dfs_moti, plt, sns):
    # For Motivation:
    ...
    plt.tight_layout()
    plt.gca()
    return metric_label_moti, metric_labels_moti, policies_moti, policy_moti
