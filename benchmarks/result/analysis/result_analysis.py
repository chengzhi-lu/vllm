...
@app.cell
def __(e2e_result_dir_names, json, os, pd):
    e2e_result_dfs = {}
...
def e2e_result(e2e_result_dfs, pd, plt, sns):
    e2e_result = {"scheduling_policies": [], "swap_policies": [], "throughput": []}
...
def __(pd, plt, selected_result, sns):
    ...
    itl_df = (
        itl_df.groupby(["scheduler_policy", "swap_policy"]).mean().reset_index()
    )
...
def __(pd, plt, selected_result, sns):
    ...
    _lat_df = (
        _lat_df.groupby(["scheduler_policy", "swap_policy"]).mean().reset_index()
    )
...
def __(os, pd, tp_dir_names):
    infer_result = pd.read_csv(
        os.path.join(
            tp_dir_names[0], "1000.0qps-Llama-2-13b-chat-hf-132949-infer.csv"
        )
    )
    fcfs_result = pd.read_csv(
        os.path.join(
            tp_dir_names[0], "1000.0qps-Llama-2-13b-chat-hf-074220-fcfs.csv"
        )
    )
