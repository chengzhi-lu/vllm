...
start_server() {
  ...
  case "$parallel_type" in
    "single")
      ...
      ;;
    "pp")
      parallel_args="--pipeline-parallel-size $num_gpus --distributed-executor-backend=ray"
      ;;
    "tp")
      parallel_args="--tensor-parallel-size $num_gpus --distributed-executor-backend=ray"
      ;;
  esac

  case "$model_name" in 
    "meta-llama/Llama-2-13b-chat-hf")
      prefill_predictor_model_config="/root/vllm/train/MODEL/results/opt-125m-llama2-13b-sharegpt-score-trainbucket10-b32/usage_config.json"
       ;;
    "meta-llama/Llama-2-13b-chat-hf")
      prefill_predictor_model_config="/root/vllm/train/MODEL/results/opt-350m-llama2-70b-sharegpt-score-trainbucket10-b32/usage_config.json"
       ;;
  esac
  ...
  CUDA_VISIBLE_DEVICES=$gpu_devices RAY_DEDUP_LOGS=0 taskset -c 28-29 python3 -m vllm.entrypoints.openai.api_server \
    ...
    --prefill-predictor-model-config  "$prefill_predictor_model_config" \
