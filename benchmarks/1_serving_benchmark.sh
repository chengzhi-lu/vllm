...
for policy in "${scheduler_policy[@]}"; do
    ...
    python3 benchmark_serving.py --dataset-path $dataset_path  --dataset-name $dataset_name --request-rate 10 --num-prompts 1000 --sharegpt-output-len 1000 --model $model_name --result-dir $result_dir --scheduler-policy $policy --save-result >> benchmark.log 2>&1
    kill $pid
    sleep 20
done